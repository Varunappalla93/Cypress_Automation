/// <reference types="Cypress"/>

describe("Inspect items using chain of commands for automation test store", () => {
    it("click on first item using item header", () => {

        cy.visit("https://automationteststore.com/")
        cy.get('#block_frame_featured_1769 > .thumbnails > :nth-child(1) > .fixed_wrapper > .fixed > .prdocutname').click()


    })

    it.only("click on first item using item text", () => {

        cy.visit("https://automationteststore.com/")
        cy.get('.prdocutname').contains('Skinsheen Bronzer Stick').click().then((itemHeaderText)=>
        {
                cy.log("Selected the item "+itemHeaderText.text()) // text() - jquery command
        })

    })

    it("click on first item using item index", () => {

        cy.visit("https://automationteststore.com/")
        cy.get('div[class="fixed_wrapper"]').find('.prdocutname').eq(0).click()

    })
})